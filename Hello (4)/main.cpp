...
//return the index of vector when given postcode
int search_gp(const std::vector<Table_gp>& gp, std::string postcode){
...
int main(){
  ...
  if (gp1.vaccine != 0){
    ...
    std::cout << "Text all nearby users, in this example, text : " << std::endl;

    for(int i = 0; i < num_to_text.size(); i++){
      std::cout << num_to_text[i] << std::endl;
    }
  }
  //
  ...
  //
  std::cout  << std::endl << "Run [signup.cpp]..." << std::endl << std::endl;
  ...
  if (signup == "YES"){
    ...
    std::cout << "Updated GP database (gpdetailsdone.txt) :" << std::endl << std::endl;

    gp_db1[search_gp(gp_db1, gp1.postcode)].vaccine = gp1.vaccine;

    std::cout << "Postcode" << "\t" << "No of vaccines available" << std::endl;

    std::ofstream outfile2;
    outfile2.open("gpdetails.txt");
    ...
    for(int i = 0; i < gp_db1.size(); i++){
      outfile2 << gp_db1[i].postcode << "\t" << gp_db1[i].vaccine << std::endl;
    }
    outfile2.close();
  ...
  //when vaccines run out
  if (gp1.vaccine == 0){
    std::cout << std::endl;
    std::cout << "Text all nearby users" << std::endl << std::endl;
    std::cout << "Vaccines are fully booked!" << std::endl << std::endl;
  }
  else{
    std::cout << std::endl << "Back to notify users, but different postcode this time, stop when reach the end of table" << std::endl << std::endl;
  }
}